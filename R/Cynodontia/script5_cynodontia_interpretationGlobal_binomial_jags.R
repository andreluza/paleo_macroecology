...
# save
ggsave (here ("output","figures", "density_Div.pdf"),
        p_div,
        width=8,height =3)
...
# psi
png(here ("output", "figures", "psi.png"),units = "cm",width=20,height=25,res=300)

par (mfrow=c(1,3))
hist(cynodonts_output$mean$muZW,main = "Cynodonts (n=500)",xlab="")
hist(mammaliaformes_output$mean$muZW,main = "Mammaliaformes (n=400)",xlab="")
hist(mammalia_output$mean$muZW,main = "Mammals (n=800)",xlab="")
...
ggsave (here ("output","figures", "p_rates.pdf"),
        p_rates,
        width=10,height =5)
...
plot1 <- ggplot (data = time_covariates %>%
                  cbind (group = 1,
                   ...
                   melt (id.vars = c("stage","group","mid_ma","max_ma","min_ma","cols_strip")),
                
                aes(x=mid_ma,y=value, #fill=temperature,col=temperature,
                    group = group))+
...
ggsave(here ("output", "figures", "covariates.png"),plot1,
       width=10,height=7)
...
dat <-   rbind (
  ...
  # remove the last that is not considered in the dynamics (t-1)
...
ggsave (here ("output", "figures", "comparison.pdf"),
        a,
        width=6,height=4)  
...
ggsave (here ("output", "figures","convergence_total_richness.png"),width =5,height=3)
